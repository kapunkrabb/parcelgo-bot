...
"""
database.py — SQLite хранилище для ParcelGold Bot
"""
import sqlite3
import os
...
class Database:
    ...
    def find_travelers(self, from_city: str, to_city: str, weight: float) -> list:
        """Ищем попутчиков по маршруту и весу"""
        rows = self.conn.execute("""
            ...
            WHERE t.status = 'active'
              ...
              AND t.weight >= ?
            ORDER BY u.rating DESC, u.trips_count DESC
        """, (f"%{from_city}%", f"%{to_city}%", weight)).fetchall()
        return [dict(r) for r in rows]
    ...
    def add_request(self, user_id, from_city, to_city, weight, desc, budget) -> int:
        ...
        self.conn.commit()
        # Уведомляем попутчиков с подходящим маршрутом
        return cur.lastrowid
    ...
    def find_matches_for_trip(self, from_city: str, to_city: str, max_weight: float) -> list:
        """Заявки отправителей, подходящие под маршрут попутчика"""
        rows = self.conn.execute("""
            SELECT r.*, u.username
            FROM requests r JOIN users u ON r.user_id = u.id
        ...
        """, (f"%{from_city}%", f"%{to_city}%", max_weight)).fetchall()
        return [dict(r) for r in rows]
